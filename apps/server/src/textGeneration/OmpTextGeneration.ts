    ...
    Effect.gen(function* () {
      ...
      if (model.length === 0) {
        return yield* new TextGenerationError({
          operation,
          detail: "OMP text generation requires a discovered or custom provider/model selection.",
        });
      }
      const outputRef = yield* Ref.make("");
      const generated = yield* Effect.scoped(
        Effect.gen(function* () {
          const launchArgs = ["--mode", "rpc", "--no-session"];
          if (ompSettings.profile.length > 0) {
            launchArgs.push("--profile", ompSettings.profile);
          }
          launchArgs.push("--model", model);
          const modelSeparator = model.indexOf("/");
          if (modelSeparator > 0) {
            launchArgs.push("--models", `${model.slice(0, modelSeparator)}/*`);
          }
          launchArgs.push(...tokenizeCliArgs(ompSettings.launchArgs));
          ...
          yield* Stream.fromQueue(client.events).pipe(
            Stream.takeWhile((event) => event.type !== "agent_end" || event.isTerminal === false),
