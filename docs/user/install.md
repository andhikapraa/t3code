...
| OMP        | [Oh My Pi](https://github.com/can1357/oh-my-pi)       | `omp`          | Profile / provider key |

Cursor is the one to watch: install Cursor CLI, which provides the `cursor-agent` binary that
T3 Code looks for, but authenticate with `agent login`, not `cursor-agent login`.

OMP supports profile-specific provider credentials rather than one universal login command. See
[OMP](./providers-omp.md) for installation, profiles, and instance configuration.
